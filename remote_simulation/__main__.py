...
def _log_goodput(sample_node) -> None:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 4. 手动连接网络
        actual_connect_peers = min(simulation_config.connect_peers, max(0, len(nodes) - 1))
        if actual_connect_peers > 0:
            topology = NetworkTopology.generate_random_topology(len(nodes), actual_connect_peers, latency_max=0)
            ...
            nodes = connect_nodes(nodes, topology, min_peers=max(0, actual_connect_peers - 2), max_workers=1000)
        ...
        run_error = exc
        logger.exception(f"模拟运行过程中发生异常: {exc}")
    finally:
        ...
        try:
            ...
            logger.warning(f"节点日志收集遇到问题: {exc}")

        hosts_log_path = f"{log_path}/hosts"
        Path(hosts_log_path).mkdir(parents=True, exist_ok=True)

        try:
            collect_host_diagnostics(host_specs, hosts_log_path)
            logger.info("主机诊断日志收集完毕")
        except Exception as exc:
            logger.warning(f"主机诊断日志收集遇到问题: {exc}")
        ...
        if args.capture_pcap:
            ...
            try:
                ...
                logger.warning(f"pcap 收集遇到问题: {exc}")
