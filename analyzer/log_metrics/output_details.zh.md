...
### 2.1 指标命名：`module::key.stat`

指标采用**三级命名**：`module::key.stat`。

- **module**：发出该指标的 Conflux 子系统（匹配 `[0-9a-z_]+`）。
- **key**：该模块内的具体度量。
- **stat**：统计后缀，表示同一指标的不同统计视角。典型的 stat 包括 `.m1`（1 分钟滑动时间平均）、`.m5`（5 分钟滑动时间平均）、`.mean`、`.p90` 等。

完整形式为 `module::key.stat`。裸 `key` 仅在该 key 在所有模块中唯一时可用；若跨模块重名则触发歧义异常。
...
在绘图和统计之前，预处理会为所有以 `.count` 结尾的指标生成 `<original>.m1`。派生原因如下：

- Conflux-Rust 代码中 **Counter** 类型的指标只输出 `.count`，不自带 `.m1`，因此脚本需要为其派生 `.m1`。
- **Meter** 类型的指标自身已包含 `.m1`，但同样也有 `.count` 字段。脚本不区分指标来自 Counter 还是 Meter，因此也会为 Meter 的 `.count` 派生出 `.count.m1`。此时 `.count.m1` 没有实际意义（与 Meter 原生的 `.m1` 重复），但不影响正确性。

`.m1` 的值是计数增量的时间衰减加权平均，本质上是一个平滑的短期速率/吞吐信号，与原始累计计数 `.count` 相对应。
